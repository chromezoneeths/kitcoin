...
async function session(ws: WebSocket, _request): Promise<void> {
	...
	let info: userActions.Info;
	info.auth = auth.auth;
	const ping = setInterval(() => {
		ws.send(JSON.stringify({action: 'ping'}));
	}, 500);
	if (userQuery) {
		info.id = userQuery.uuid;
		info.address = userQuery.address;
		// UserID = userQuery.uuid;
		// address = userQuery.address;
		info.name = userQuery.name;
		// Role = userQuery.role;
		info.role = userQuery.role;
	} else {
		info.id = uuid();
		info.role = db.Permission.student;
		await db.addUser(info.id, user.data.emailAddresses[0].value, user.data.names[0].displayName).catch(error => {
			console.log(`RECORDS, ERROR: ${error}`);
		});
		info.address = user.data.emailAddresses[0].value;
		info.name = user.data.names[0].displayName;
	}
